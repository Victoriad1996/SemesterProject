from brian2 import *
import BasicFunctions, ConvertingFunctions, PlotVoltagesFunctions, CellActivityFunctions
...
class Model:
    def __init__(self, param: dict, model="FairhallModel", plasticity=True):
        ...
        self.dict_group_neurons = {}
        self.dict_synapses = {'PC': {}, 'INH' : {}}
        ...
        self.dict_group_neurons['P'] = self._init_poisson_group()
        self.dict_group_neurons['R'] = self._init_random_input()
        ...
        if self.model == "FairhallModel":
            self.dict_group_neurons['PC'] = self._init_pyramidal_neurons_fairhall()
            if self.plasticity:
                self.dict_synapses['PC']['SPCP'], self.dict_synapses['PC']['SPCP2'] = self._init_poisson_synapses_fairhall_plastic()
            else:
                self.dict_synapses['PC']['SPCP'], self.dict_synapses['PC']['SPCP2'] = self._init_poisson_synapses_fairhall_not_plastic()

            # self._init_noise_synapses_fairhall()

        elif self.model == "ThresholdModel":
            if self.plasticity:
                self.dict_group_neurons['PC'] = self._init_pyramidal_neurons_threshold_plastic()
            else:
                self.dict_group_neurons['PC'] = self._init_pyramidal_neurons_threshold_not_plastic()

            self.dict_synapses['PC']['SPCP'] = self._init_poisson_synapses_threshold()
            # self._init_noise_synapses_threshold()

        if 'inputs' in self.p.keys():
            self.dict_group_neurons['INPUT'] = self._init_input_neurons()
            self.dict_synapses['PC']['SPCINPUT'] = self._init_input_synapses()

        self.dict_group_neurons['INH'] = self._init_inhibitory_neurons()

        self.dict_synapses['PC']['SPCINH'], self.dict_synapses['INH']['SINHPC'] = self._init_standard_synapses()
        self.dict_synapses['PC']['SRPC'] = self._init_random_synapses()
        # Trajectory
        ...
        self.neuron_idx = 50
        self.my_indices = BasicFunctions.some_indices(self.neuron_idx)
    ...
    def get_dict_group_neurons(self):
        ...
        return self.dict_group_neurons

    def get_dict_synapses(self):

        return self.dict_synapses
    ...
    def _complete_parameters(self, p: dict):
        ...
        genericParam = {
            ...
            # Excitatory neurons
            "v_init_exc": -68,  # Initial value of potential
            ...
            "gi_inh": 1,  # ???
            "inh_weight_pi": 0.03,  # 0.03 FairhallPaper
            "inh_weight_ip": 0.02,  # 0.02 FairhallPaper
            #
            # # Noise Neurons
    ...
    def _init_pyramidal_neurons_fairhall(self):
        ...
        # Initialises voltage
        self.PC.v = ConvertingFunctions.convert_list_to_threshold(self.p['trajectory'][0, :], self.p['v_init_exc'] + 15,
                                                                  self.p['v_init_exc'])
        ...
        #                                               self.p['v_thr_exc'])
        self.PC.I = ConvertingFunctions.convert_list_to_threshold(self.p['trajectory'][0, :], -45, -68)
    ...
    def _init_pyramidal_neurons_threshold_plastic(self):
        ...
        # Init the threshold
        self.PC.h = ConvertingFunctions.convert_list_to_threshold(self.p['trajectory'][0, :],
                                                                  self.p['treshold_trajectory'],
                                                                  self.p['treshold_outside_trajectory'])
        self.PC.threshold_leak = ConvertingFunctions.convert_list_to_threshold(self.p['trajectory'][0, :],
                                                                               self.p['treshold_trajectory'],
                                                                               self.p['treshold_outside_trajectory'])
    ...
    def _init_input_synapses(self):
        ...
        sources, targets = ConvertingFunctions.convert_matrix_to_source_target(self.p['inputs'])
        self.SPCINPUT.connect(i=targets, j=targets)
        self.SPCINPUT.w = 2 * self.p["INPUT_weight"]
        return self.SPCINPUT
    ...
    def _init_random_input(self):
        ...
        tau : second
        '''
        v_thr_random = self.p['v_thr_random']
        ...
        self.R.v = -80
        self.R.v_thr_random = self.p['v_thr_random']
        return self.R
    ...
    def _init_standard_synapses(self):
        ...
        # self.SPC.connect()
        self.SPC.w = 'weight * exp( - ( ( x_pre - x_post )**2 + ( y_pre - y_post )**2 ) / ( lambda_PC * metre ) **2 )'
        ...
        return self.SPCINH, self.SINHPC

    def _init_poisson_synapses_threshold(self):
        self.SPCP = Synapses(self.P, self.PC, 'w:1', on_pre='v_post+=w')

        sources, targets = ConvertingFunctions.convert_matrix_to_source_target(self.p['trajectory'])
        # self.SPCP.connect()
    ...
    def _init_poisson_synapses_fairhall_plastic(self):
        ...
        self.SPCP2 = Synapses(self.P, self.PC, eqs_weight, on_pre='''v_post+=w''',
                              on_post='''w = clip(w + 0.5, 0, 4)''')
        sources, targets = ConvertingFunctions.convert_neg_matrix_to_source_target(self.p['trajectory'])
        self.SPCP2.connect(i=targets, j=targets)
        self.SPCP2.w = self.p["P_weight"]
        self.SPCP2.w_leak = self.p["P_weight"]
        return self.SPCP, self.SPCP2
    ...
    def _init_noise_synapses_fairhall(self):
        ...
        # For connection
        sources, targets = ConvertingFunctions.convert_matrix_to_source_target(connection_matrix)
        self.SPCN2.connect(i=sources, j=targets, p=0.5)

        # Weight of the synapses
        self.SPCN2.w = self.p['noise_weight']
        return self.SPCN, self.SPCN2
    ...
    def run(self, duration=50 * ms, show_PC=False, show_other=False, record_=True, plot_last_first=False):
        ...
        if record_:
            ...
            if show_other:
                PlotVoltagesFunctions.plot_voltages_other_types(self)

        CellActivityFunctions.spiking_times_fun(self)

        if show_PC:
            PlotVoltagesFunctions.plot_voltages_PC(self)
